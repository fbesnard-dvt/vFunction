...
public abstract class BaseUserServiceImpl<T extends User> extends
        CriteriaPersistenceService {

    @SuppressWarnings("unchecked")
    protected final Class<T> entityClass = ClassUtils
    ...
    protected boolean authenticateUser(String username, String password) {
        int number = CriteriaPersistenceSupport.count(entityManager,
                criteriaBuilder, entityClass, PredicationFactory
                        .createEqualPredication(username, User_.username),
                PredicationFactory.createEqualPredication(password,
                        User_.password));
        return (number == 1);
    }
    ...
    private void isDuplicateUser(T user) throws DuplicateUsernameException {
        ...
        if (CriteriaPersistenceSupport.count(entityManager, criteriaBuilder,
                entityClass, PredicationFactory.createEqualPredication(
                        user.getUsername(), User_.username)) > 0) {
