...
public class PageContextImpl implements PageContext {
    ...
    public void invalidateSession() {
        ...
        Object session = getFacesContext().getExternalContext().getSession(
                false);
        try {
            session.getClass().getMethod(INVALIDATE_SESSION_METHOD_NAME)
                    .invoke(session);
        } catch (IllegalAccessException e) {
            throw new SystemException("Cannot invalidate session", e);
        } catch (InvocationTargetException e) {
            throw new SystemException("Cannot invalidate session",
                    e.getTargetException());
        } catch (NoSuchMethodException e) {
            throw new AssertionError(
                    "Cannot invalidate session, because the session of class "
                            + session.getClass().getName()
                            + " doesn't contain method '"
                            + INVALIDATE_SESSION_METHOD_NAME + "'");
        }
    }
