 ...
 */
public class OpenBrowserServletContextListener
  ...
  private void regesterServerStartUpListener() {
    ...
    try {
      mbeanServer = (MBeanServer)new InitialContext().lookup(
          "java:comp/env/jmx/runtime");

      // Create Objectname for the runtime service
      ObjectName runtimeService = new ObjectName("com.bea:Name=RuntimeService,Type=" +
    ...
    try {
      ...
      httpURL = (String)mbeanServer.invoke(serverRuntime, "getURL", 
        new Object[] { "http" }, new String [] { "java.lang.String" }); 
    } catch (Exception e) {
      log("Failed to get listen URL for http protocol, so use the default one " + httpURL 
    ...
    try {
      ...
      if (RUNNING_STATE.equalsIgnoreCase(serverState)) {
        ...
        mbeanServer.addNotificationListener(serverRuntime, 
      ...
      log("Failed to register listener to ServerRuntime Mbean for openning browser: " + 
          e.getMessage());
    } catch (Exception e){
      log("Failed to browser to " + httpURL + contextPath+ "/" + page + ". Error message is:" 
          + e.getMessage());
    }
