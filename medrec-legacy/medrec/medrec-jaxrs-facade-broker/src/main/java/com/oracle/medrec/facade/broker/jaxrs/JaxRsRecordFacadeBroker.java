...
public class JaxRsRecordFacadeBroker {
    ...
    private static final Logger LOGGER = Logger
            .getLogger(JaxRsRecordFacadeBroker.class.getName());
    ...
    public Response createRecord(RecordToCreate recordToCreate) {
        Response rs = null;
        try {
            recordFacade.createRecord(recordToCreate);
            LOGGER.info("Create new Record.");
            rs = Response.status(Response.Status.CREATED).build();
        } catch (Exception ex) {
            rs = Response.status(Response.Status.INTERNAL_SERVER_ERROR)
    ...
    public RecordSummary getRecordSummaryByPatientId(
            @QueryParam("patientId") Long patientId) {
        try {
            RecordSummary rs = recordFacade
                    .getRecordSummaryByPatientId(patientId);
            return rs;
        } catch (Exception e) {
