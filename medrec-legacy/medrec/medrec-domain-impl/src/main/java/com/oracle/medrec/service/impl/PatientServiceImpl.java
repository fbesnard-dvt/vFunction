 ...
 */
@Stateless
public class PatientServiceImpl extends BaseUserServiceImpl<Patient> implements
        PatientService {
    ...
    public void createPatient(Patient patient)
            throws DuplicateUsernameException, DuplicateSsnException {
        isDuplicateSsn(patient);
        super.createUser(patient);
    }

    public Patient getPatient(Long patientId) {
        return entityManager.find(entityClass, patientId);
    }

    public Patient findApprovedPatientBySsn(String ssn) {
        return CriteriaPersistenceSupport.findUnique(entityManager,
                criteriaBuilder, entityClass, PredicationFactory
                        .createEqualPredication(ssn, "ssn"), PredicationFactory
                        .createEqualPredication(Patient.Status.APPROVED,
                                "status"));
    }

    @TransactionAttribute(TransactionAttributeType.NOT_SUPPORTED)
    public List<Patient> findApprovedPatientsByLastName(String lastName) {
        return CriteriaPersistenceSupport.find(entityManager, criteriaBuilder,
                entityClass, PredicationFactory.createEqualPredication(
                        lastName, "name", "lastName"), PredicationFactory
                        .createEqualPredication(Patient.Status.APPROVED,
                                "status"));
    }

    public boolean authenticatePatient(String username, String password) {
        int number = CriteriaPersistenceSupport
                .count(entityManager, criteriaBuilder, entityClass,
                        PredicationFactory.createEqualPredication(username,
                                "username"), PredicationFactory
                                .createEqualPredication(password, "password"),
                        PredicationFactory.createEqualPredication(
                                Patient.Status.APPROVED, "status"));
        return (number == 1);
    }

    public Patient authenticateAndReturnPatient(String username, String password) {
        return CriteriaPersistenceSupport
                .findUnique(entityManager, criteriaBuilder, entityClass,
                        PredicationFactory.createEqualPredication(username,
                                "username"), PredicationFactory
                                .createEqualPredication(password, "password"),
                        PredicationFactory.createEqualPredication(
    ...
    public List<Patient> getNewlyRegisteredPatients() {
        return CriteriaPersistenceSupport.find(entityManager, criteriaBuilder,
                entityClass, PredicationFactory.createEqualPredication(
                        Patient.Status.REGISTERED, "status"));
    }
    ...
    @TransactionAttribute(TransactionAttributeType.NOT_SUPPORTED)
    public List<Patient> fuzzyFindApprovedPatientsByLastnameAndSsn(
            String lastName, String ssn) {
        return CriteriaPersistenceSupport.find(entityManager, criteriaBuilder,
                entityClass, 0, 10, PredicationFactory.createEqualPredication(
                        Patient.Status.APPROVED, "status"), PredicationFactory
                        .createLikePredication(lastName + "%", "name",
                                "lastName"), PredicationFactory
                        .createLikePredication(ssn + "%", "ssn"));
    }
    ...
    private void isDuplicateSsn(Patient patient) throws DuplicateSsnException {
        ...
        int ssnExistedAmount = CriteriaPersistenceSupport.count(entityManager,
                criteriaBuilder, entityClass, PredicationFactory
                        .createEqualPredication(patient.getSsn(), "ssn"));
